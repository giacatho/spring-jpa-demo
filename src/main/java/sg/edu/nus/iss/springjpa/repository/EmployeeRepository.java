...
import sg.edu.nus.iss.springjpa.model.Employee;
...
public interface EmployeeRepository extends JpaRepository<Employee, String> {
  ...
  @Query("SELECT DISTINCT m FROM Employee e, Employee m " +
                            " where e.managerId = m.employeeId ")
  List<Employee> findAllManagers();

  @Query("SELECT DISTINCT m.name FROM Employee e, " +
             " Employee m where e.managerId = m.employeeId ")
  List<String> findAllManagerNames();
   
  @Query("SELECT DISTINCT e2 FROM Employee e1, Employee e2 " +
         " WHERE e1.employeeId = e2.managerId AND e1.employeeId = :eid")
